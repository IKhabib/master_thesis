...
- Added a substantive abstract and separated author metadata into
  `author_details.tex`. Shortened list entries while retaining full
...
`python build.py` for local backend selection, or follow `README_BUILD.md`.

## Information still required from the author

Fill in the five marked personal/institutional fields in
`author_details.tex`, confirm the degree and department, and add any
