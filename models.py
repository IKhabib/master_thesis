...
Includes:
1. Bias-Variance decomposition with composite bar graphs
2. Coefficient distributions (histograms of first 3 basis coefficients)
3. β(s) distributions at selected points
"""
...
def plot_coefficient_distributions(beta_estimates_dict: Dict, v: np.ndarray,
    ...
    try:
        fig.savefig(f"{save_dir}/{model_name.lower().replace(' ', '_')}_coeff_distributions.pdf",
                    dpi=300, bbox_inches='tight')
        print(f"  Saved: {model_name.lower().replace(' ', '_')}_coeff_distributions.pdf")
    except:
        pass
    plt.close(fig)
...
def plot_beta_at_points(beta_estimates_dict: Dict, s: np.ndarray,
    ...
    try:
        fig.savefig(f"{save_dir}/{model_name.lower().replace(' ', '_')}_beta_point_distributions.pdf",
                    dpi=300, bbox_inches='tight')
        print(f"  Saved: {model_name.lower().replace(' ', '_')}_beta_point_distributions.pdf")
    except:
        pass
    plt.close(fig)
...
def plot_coefficient_comparison_across_models(all_results: Dict, v: np.ndarray,
    ...
    try:
        fig.savefig(f"{save_dir}/coeff_distributions_across_models.pdf",
                    dpi=300, bbox_inches='tight')
        print(f"  Saved: coeff_distributions_across_models.pdf")
    except:
        pass
    plt.close(fig)
...
def plot_beta_distribution_across_models(all_results: Dict, s: np.ndarray,
    ...
    try:
        fig.savefig(f"{save_dir}/beta_distributions_across_models.pdf",
                    dpi=300, bbox_inches='tight')
        print(f"  Saved: beta_distributions_across_models.pdf")
    except:
        pass
    plt.close(fig)
...
def run_full_simulation_with_bias_variance(M: int = 100, test_mode: bool = True):
    ...
    for model_name, lam, beta in models:
        ...
        # Plot coefficient distributions (first 3 cosine coefficients)
        plot_coefficient_distributions(
            beta_estimates, v, beta, method_names, model_name,
            J=10, n_coeffs=3, save_dir='.'
        )

        # Plot β(s) distributions at selected points
        plot_beta_at_points(
    ...
    # Coefficient distributions across models
    plot_coefficient_comparison_across_models(
        all_results, v, method_names, model_names, J=10, n_coeffs=3, save_dir='.'
    )

    plot_beta_distribution_across_models(
        all_results, s, method_names, model_names, n_points=5, save_dir='.'
    )
...
if __name__ == "__main__":
    ...
    results = run_full_simulation_with_bias_variance(M=5000, test_mode=False)
